...
import { parseWorkbookBuffer } from "@/lib/parseWorkbook";
...
export function Dropzone({
  ...
  onParsed: (sheets: ParsedSheet[], fileName: string) => void;
  onLoadDemo: () => void;
  usingDemo: boolean;
}) {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      if (withRows.length === 0) {
        setError("В файле не найдено ни одной строки с турами. Проверьте столбцы: имя / маршрут / № тура / дата / мест в продажу / продано.");
        return;
      }
    ...
    <div className="card p-6 flex flex-col gap-4">
      ...
      >
        ...
        <span className="text-[12px]" style={{ color: "var(--text-muted)" }}>
          Столбцы: имя / маршрут / № тура / дата / мест в продажу / продано — порядок и написание могут отличаться
        </span>
        <input
          ref={inputRef}
          type="file"
